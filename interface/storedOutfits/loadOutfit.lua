...
local createIcon = function(outfit)
  ...
  local encDir = config.encrypt.directives
  ...
  -- Return early if name encryption is enabled.
  if config.encrypt.name then return icon end
  ...
  if head then
    ...
    if frames then
      itemFrames[5] = { image = cfg.directory .. frames .. ":normal" .. (not encDir and head.parameters and head.parameters.directives or ""), position = {16, 0} }
    end
  end
  ...
  if chest then
    ...
    if frames then
      if frames.backSleeve then
        itemFrames[1] = { image = cfg.directory .. frames.backSleeve .. ":idle.1" .. (not encDir and chest.parameters and chest.parameters.directives or ""), position = {16, 0} }
      end
      if frames.body then
        itemFrames[4] = { image = cfg.directory .. frames.body .. ":idle.1" .. (not encDir and chest.parameters and chest.parameters.directives or ""), position = {16, 0} }
      end
      if frames.frontSleeve then
        itemFrames[6] = { image = cfg.directory .. frames.frontSleeve .. ":idle.1" .. (not encDir and chest.parameters and chest.parameters.directives or ""), position = {16, 0} }
      end
    end
  end
  ...
  if legs then
    ...
    local frames = getFrames(cfg)
    if frames then
      itemFrames[3] = { image = cfg.directory .. frames .. ":idle.1" .. (not encDir and legs.parameters and legs.parameters.directives or ""), position = {16, 0} }
    end
  end
  ...
  if back then
    ...
    local frames = getFrames(cfg)
    if frames then
      itemFrames[2] = { image = cfg.directory .. frames .. ":idle.1" .. (not encDir and back.parameters and back.parameters.directives or ""), position = {16, 0} }
    end
  end
